...
function xSGroups.Database.UpdateUsersGroup(userid, rank)
	xAdmin.Database.Query(string.format("INSERT INTO %s_secondary_user (userid, `rank`) VALUES ('%s', '%s') ON DUPLICATE KEY UPDATE `rank`='%s';", xAdmin.Config.Name, userid, xAdmin.Database.Escape(rank), xAdmin.Database.Escape(rank)))
end

function xSGroups.Database.GetUsersGroup(userid, callback)
	xAdmin.Database.Query(string.format("SELECT * FROM %s_secondary_user WHERE userid='%s';", xAdmin.Config.Name, userid), callback)
end
